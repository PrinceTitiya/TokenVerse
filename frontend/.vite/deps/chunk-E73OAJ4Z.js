...
var InvalidHexBooleanError2 = class extends BaseError3 {
  constructor(hex) {
...
function hexToBool(hex_, opts = {}) {
  ...
  if (trim3(hex) === "0x01")
    return true;
  throw new InvalidHexBooleanError2(hex);
}
...
function keccak256(value, to_) {
  const to = to_ || "hex";
  const bytes = keccak_256(isHex(value, { strict: false }) ? toBytes3(value) : value);
  if (to === "bytes")
    return bytes;
  return toHex2(bytes);
}
...
var hash = (value) => keccak256(toBytes3(value));
...
var SocketClosedError = class extends BaseError3 {
  constructor({ url } = {}) {
    super("The socket has been closed.", {
      metaMessages: [url && `URL: ${getUrl2(url)}`].filter(Boolean),
...
async function call(client, args) {
  ...
  const { account: account_ = client.account, authorizationList, batch = Boolean((_a = client.batch) == null ? void 0 : _a.multicall), blockHash, blockNumber, blockTag = client.experimental_blockTag ?? "latest", requireCanonical, accessList, blobs, blockOverrides, code, data: data_, factory, factoryData, gas, gasPrice, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, nonce, requestOptions, to, value, stateOverride, ...rest } = args;
  const account = account_ ? parseAccount(account_) : void 0;
  ...
  try {
    ...
    if (batch && shouldPerformMulticall({ request }) && !rpcBlockOverrides && blockHash === void 0) {
      try {
        const { deployless = false } = typeof ((_e = client.batch) == null ? void 0 : _e.multicall) === "object" ? client.batch.multicall : {};
    ...
    if ((_f = requestOptions == null ? void 0 : requestOptions.signal) == null ? void 0 : _f.aborted)
      throw getAbortError(requestOptions.signal);
    if (isAbortError(err))
      throw err;
    const data2 = getRevertErrorData(err);
    const { offchainLookup: offchainLookup2, offchainLookupSignature: offchainLookupSignature2 } = await import("./ccip-I73PPJWF.js");
...
//# sourceMappingURL=chunk-E73OAJ4Z.js.map
