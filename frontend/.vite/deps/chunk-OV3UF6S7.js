...
async function getTransaction(client, { blockHash, blockNumber, blockTag: blockTag_, hash: hash2, index, sender, nonce }) {
  var _a, _b, _c;
        ...
        try {
          ...
          if (err instanceof TransactionNotFoundError || err instanceof TransactionReceiptNotFoundError) {
            ...
            try {
              ...
              retrying = false;
              const replacementTransaction = block.transactions.find(({ from: from12, nonce }) => from12 === replacedTransaction.from && nonce === replacedTransaction.nonce);
...
async function recoverPublicKey({ hash: hash2, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-BIWJLKVK.js");
  const signature_ = (() => {
...
async function fillTransaction(client, parameters) {
  var _a, _b, _c, _d, _e;
  const { account = client.account, accessList, authorizationList, chain = client.chain, blobVersionedHashes, blobs, data, gas, gasPrice, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, nonce: nonce_, nonceManager: nonceManager2, to: to2, type, value, ...rest } = parameters;
  ...
  try {
    ...
    const format4 = ((_d = (_c = chain == null ? void 0 : chain.formatters) == null ? void 0 : _c.transaction) == null ? void 0 : _d.format) || formatTransaction;
...
async function prepareTransactionRequest(client, args) {
  ...
  const fillResult = attemptFill ? await getAction(client, fillTransaction, "fillTransaction")({ ...request, nonce }).then((result) => {
    const { chainId: chainId2, from: from12, gas: gas2, gasPrice, nonce: nonce2, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, type: type2, ...rest } = result.transaction;
...
function hmac256(key, value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = hmac(sha2562, from(key), from(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes(bytes);
}
...
function verify(options) {
  ...
  if (options.address)
    return isEqual(options.address, recoverAddress2({ payload, signature: options.signature }));
  return secp256k1.verify(options.signature, from(payload), toBytes2(options.publicKey), ...hash2 ? [{ prehash: true, lowS: true }] : []);
}
...
function buildRequest(request, options = {}) {
  return async (args, overrideOptions = {}) => {
    ...
    const requestId = dedupe ? hashString(`${uid}.${stringify(args)}`) : void 0;
...
async function sendTransaction(client, parameters) {
  ...
  try {
    ...
    if (nonceManagerParameters)
      (_f = account == null ? void 0 : account.nonceManager) == null ? void 0 : _f.reset(nonceManagerParameters);
    throw getTransactionError(err, {
      ...parameters,
      account,
...
export {
  ...
  fromHex3 as fromHex2,
  Rlp_exports,
  assert3 as assert2,
  fromHex4 as fromHex3,
  extract2 as extract,
  from7 as from4,
  fromRpc,
  fromTuple,
...
//# sourceMappingURL=chunk-OV3UF6S7.js.map
